...
**"Eingereicht" sperrt nichts:** weder die Buttons in der Tagesliste (§9) noch die RLS-Policy auf `availability_entries` prüfen `submitted_at` — ein Mitarbeiter kann seine Angaben jederzeit ändern, auch nach dem Einreichen und auch nach dem Stichtag, bis der Admin die Schichten tatsächlich zuweist. Die Admin-Planung liest bei jedem Laden live die aktuellen `availability_entries`, es gibt keinen Schnappschuss zum Zeitpunkt des Einreichens — "Einreichen" bedeutet nur "vollständig, bereit zur Planung", nicht "eingefroren". Die Karte in Profil weist nach dem Einreichen explizit darauf hin, dass Änderungen weiterhin möglich sind.
