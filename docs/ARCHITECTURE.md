...
- **Verfügbarkeit** (`/verfuegbarkeit`, `pages/Verfuegbarkeit.tsx`, **nur für Mitarbeiter** — Route und Navbar-Eintrag sind für `role = 'admin'` komplett ausgeblendet): eigener Tab statt Teil des Profils (vorher dort unten in einer einzigen langen Karte — Feedback: "geht neben Profilbild/Name optisch unter"). Admins müssen laut Hausregel keine Verfügbarkeit abgeben (sie werden nur im Notfall direkt in der Schichtplanung zugewiesen, §10) — sie tauchen deshalb weder in diesem Tab noch in der "Verfügbarkeit fehlt"-Hinweiskarte auf Home noch im Verfügbarkeits-Stichtag/den Erinnerungen der Admin-Planung (§10/§11) auf.
  ...
  **Bugfix: jede Eingabe sprang zurück an den Seitenanfang** (Feedback: "Wenn ich bei Verfügbarkeiten eine Eingabe mache springe ich automatisch wieder zum Anfang zurück. Das ist sehr nervig und macht die Eingabe schwer"): `load()` hat bislang bei jedem Aufruf `setLoading(true)` gesetzt — und `setDayAvailability()`/`updateDayNote()`/`setEitherOrPartner()`/`submitMonth()` riefen `load()` nach jeder einzelnen Aktion auf. Die lange Tagesliste wurde dadurch kurz durch die einzeilige `<p>Lädt…</p>` ersetzt, die Seite dabei massiv niedriger — der Browser klemmt die Scrollposition dann auf die neue, kürzere Seitenhöhe, und blieb dort stehen, obwohl die Liste unmittelbar danach wieder ihre volle Höhe zurückbekam. Fix: `load()` nimmt jetzt ein `silent`-Flag; nur der allererste Ladevorgang beim Seitenaufruf (`useEffect`) bleibt "laut" (zeigt "Lädt…"), jede Neuladung nach einer eigenen Aktion aktualisiert die bereits sichtbare Liste still an Ort und Stelle, ohne sie zwischendurch zu leeren.
